# 📮 Postman Testing Guide - RideShare API
...
- **Action:** ✅ Token auto-saves to `{{userToken}}`
...
- **Expected:** `200 OK` with JWT token
- **Action:** ✅ Token auto-saves to `{{driverToken}}`
...
- **Action:** ✅ Ride ID auto-saves to `{{rideId}}`
...
**Result:** You don't need to manually copy-paste tokens! 🎉
...
- ✅ Headers tab (for authenticated requests)

### **What to Highlight:**
- 🔵 JWT tokens in login responses
- 🔵 Ride IDs in create ride response
...
2. Check if token saved: Click environment → verify `userToken` or `driverToken` has value
...
1. Verify you're using correct token:
   - User endpoints → `{{userToken}}`
   - Driver endpoints → `{{driverToken}}`
